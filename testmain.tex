...
标准深度 Ritz 方法（Deep Ritz Method, DRM）是一种基于变分原理的 PDE 求解方法，其核心是将试探函数表示为神经网络：
\begin{equation}
u_\theta(x) = NN(x; \theta),
\end{equation}
其中 \(NN(x; \theta)\) 是一个全连接神经网络，输入为空间坐标 \(x \in \mathbb{R}^d\)，输出为标量 \(u_\theta(x)\)，\(\theta\) 是网络参数。DRM 的本质在于利用神经网络的通用逼近能力，通过最小化能量泛函来近似 PDE 的解。对于泊松方程 \(-\nabla^2 u = f\)，其损失函数为：
\begin{equation}
...
DRM 的优势在于其灵活性和对高维问题的适应性，但其局限性在于仅依赖全局坐标 \(x\)，难以捕捉解的局部几何特性，同时通过惩罚项强制满足边界条件可能导致边界处误差较大。此外，对复杂解的逼近可能需要更深的网络或更长的训练时间。这些不足为增强型方法的提出提供了动机。
这些不足促使了增强型方法的提出。

\subsection{增强机制：MLS 形函数}
增强型深度 Ritz 方法引入移动最小二乘法（MLS）形函数 \(\Psi(x)\) 来改进试探函数。MLS 形函数已在伽辽金无网格法章节详细推导，这里简述其构造：
...
\end{equation}
其中 \(x_1\) 是 \(x\) 的第 1 维坐标（在一维情况下）。对于批量输入，\(z(x)\) 的形状为 \((batchsize, 1 + N)\)。从数学意义上，\(x\) 提供全局上下文，确保网络感知整体空间位置，而 \(\Psi(x)\) 引入局部插值信息，使网络能够感知离散节点的分布以及支持域内的几何关系。拼接后的 \(z(x)\) 是一个复合特征向量，融合了全局和局部信息，从而增强了网络的表达能力。这种增强机制通过 \(\Psi(x)\) 的紧支撑性使网络对 \(x\) 附近节点的贡献更敏感，改善局部解的精度，同时形函数的单位分解性有助于将边界条件嵌入解中，减少对惩罚项的依赖。此外，拼接特征为神经网络提供了更丰富的输入空间，使其能够学习更复杂的解模式。
