...
深度 Ritz 方法在 1000 步训练时的拟合失败情况显著，如图 3.2(a) 所示，预测解（蓝色曲线）与精确解（橙色曲线）存在明显偏差。这表明模型在有限迭代步数下未能充分收敛，说明训练步数不足以优化网络参数 \(\theta\)，权重未适配短时训练。相比之下，10000 步训练（图 3.2(b)）显示预测解与精确解几乎重合，验证了深度Ritz法迭代次数的重要性。
\chapter{伽辽金无网格法}
...
\begin{figure}[ht] % 使用 [ht] 允许更灵活的浮动位置
    ...
    \end{minipage}
\end{figure}
图 \ref{CHGraphsettings_fig1_meshfree} 和表 \ref{tab:exact_vs_computed} 展示了伽辽金无网格法（EFG 方法）在相同一维抛物线特性问题上的数值结果，其中解析解为 \( u(x) = 0.5x^2 + 0.5x \)。如图 \ref{CHGraphsettings_fig1_meshfree} 所示，EFG 方法在简单一维问题上具有较高的精度。EFG 方法的绝对误差在 \( x = 0.9 \) 处达到最大值 0.0048854，平均误差约为 0.0013，整体误差保持在 \( 10^{-3} \) 数量级，精度较高。与 Deep Ritz 方法相比，EFG 方法的误差分布较为均匀。但EFG 方法的误差在边界附近（如 \( x = 0.9 \)）较大，而Deep Ritz 方法通过惩罚项（penalty=500）能更有效地约束了边界条件（\( u(0) = 0, u(1) = 1 \)）。 
...
10000 步训练后，DeepRitz 方法的 L2 误差和 H1 误差分别为 0.016862 和 0.080057，相较 1000 步（0.25 和 0.5）分别降低约 93\% 和 84\%，但中间区域 L2 误差较大，边界处导数误差显著；EFG-DeepRitz 方法的 L2 误差和 H1 误差为 0.002457 和 0.049370，相较 1000 步（0.1 和 0.2）分别降低约 97\% 和 75\%，误差分布波动较多但整体更低，优于 DeepRitz。
\newpage
\subsection*{误差收敛效果}
...
DeepRitz 方法初始 L2 误差和 H1 误差较高（2.0 和 1.75），前 200 步下降至 0.5 和 0.75，最终稳定在 0.25 和 0.5，但存在波动且导数拟合较弱；EFG-DeepRitz 方法初始误差较低（1.4-1.6），收敛更快，最终误差降至 0.1 和 0.2，训练更稳定，H1 误差与 L2 误差差距较小，显示出再生核增强对函数值和导数拟合的显著改进。
...
\begin{figure}[htbp]
    ...
    \label{fig:combined}
\end{figure}
10000 步训练后，DeepRitz 方法的 L2 误差和 H1 误差分别从 1.2 和 1.0 收敛至 0.05 和 0.2，相较 1000 步（0.25 和 0.5）降低了约 80\% 和 60\%，但导数拟合仍是瓶颈；EFG-DeepRitz 方法的 L2 误差和 H1 误差从 1.0 和 0.8 收敛至 0.02 和 0.1，相较 1000 步（0.1 和 0.2）降低了约 80\% 和 50\%，收敛更快且精度更高。EFG 增强显著改善了初始逼近和最终误差，建议优化 EFG 参数和调整惩罚项以进一步提升导数拟合。
...
\begin{figure}[ht]
    ...
    \begin{axis}[
        title={训练时间对比},
        ybar,
        bar width=0.2cm, % 减小柱宽，避免柱重叠
        enlargelimits=0.2, % 增加柱间距
        legend style={at={(0.5,-0.2)}, anchor=north,legend columns=-1}, % 图例放置在下方
        ylabel={训练时间 (秒)},
        symbolic x coords={1000 步, 10000 步, 20000 步},
        xtick=data,
        ymin=0, ymax=300, % 调整 y 轴范围
        nodes near coords, % 显示柱顶数值
        nodes near coords align={vertical},
        every node near coord/.append style={font=\scriptsize, xshift=0pt, yshift=2pt}, % 调整字体大小和位置
        width=6cm, % 缩小图表宽度
        height=6cm
    ]
    \addplot[fill=blue!50] coordinates {(1000 步, 23.95) (10000 步, 34.79) };
    \addplot[fill=green!50] coordinates {(1000 步, 3.36) (10000 步, 244.76) };
    \legend{DeepRitz 方法, EFG-DeepRitz 方法}
    \end{axis}
    \end{tikzpicture}
\begin{comment}
    \quad % 控制两图间距
    ...
    \begin{axis}[
        title={计算时间对比},
        ybar,
        bar width=0.2cm, % 减小柱宽
        enlargelimits=0.2, % 增加柱间距
        legend style={at={(0.5,-0.2)}, anchor=north,legend columns=-1}, % 图例放置在下方
        ylabel={计算时间 (秒)},
        symbolic x coords={1000 步, 10000 步, 20000 步, 伽辽金无网格},
        xtick=data,
        ymin=0, ymax=0.35, % 调整 y 轴范围
        nodes near coords, % 显示柱顶数值
        nodes near coords align={vertical},
        every node near coord/.append style={font=\scriptsize, xshift=0pt, yshift=2pt}, % 调整字体大小和位置
        width=8cm, % 调整图表宽度
    ...
    \addplot[fill=blue!50] coordinates {(1000 步, 0.001) (10000 步, 0.0015) (20000 步, 0.0020) (伽辽金无网格, 0)};
    \addplot[fill=green!50] coordinates {(1000 步, 0.01) (10000 步, 0.01) (20000 步, 0) (伽辽金无网格, 0)};
    \addplot[fill=red!50] coordinates {(1000 步, 0) (10000 步, 0) (20000 步, 0) (伽辽金无网格, 0.3)};
    \legend{DeepRitz 方法, EFG-DeepRitz 方法, 伽辽金无网格方法}
    \end{axis}
    \end{tikzpicture}
    \end{comment}
    \caption{训练时间对比}
    ...
    从图 \ref{fig:time_comparison} 可以看出，DeepRitz 方法的训练时间随步数增加显著增长（1000 步 23.95 秒，10000 步 34.79 秒，20000 步 70.88 秒），而 EFG-DeepRitz 方法训练时间更长（1000 步 3.36 秒，10000 步 244.76 秒），反映了再生核增强引入的额外计算开销。计算时间方面，DeepRitz 方法（0.001 秒至 0.002 秒）和 EFG-DeepRitz 方法（0.01 秒）均远低于伽辽金无网格方法（0.3 秒），表明深度学习方法在推理阶段的高效性。综合来看，EFG-DeepRitz 方法虽然训练时间较长，但计算时间仍具优势，适合需要快速推理的应用场景。
...
\begin{figure}[htbp]
    ...
    \label{fig:combined}
\end{figure}
RKPM 方法的 L2 误差和 H1 误差分别为 0.011709 和 0.043137，误差分布平滑但边界处导数误差较大；EFG-DeepRitz 方法的 L2 误差为 0.002368（比 RKPM 低约 80\%），H1 误差为 0.058213（高约 35\%），L2 误差分布波动较大但精度更高，导数误差波动性强。EFG-DeepRitz 结合深度学习显著提升了 L2 误差的拟合能力，但导数拟合较弱.

\chapter{结论}
本文提出的再生核增强深度 Ritz 法（EFG-DeepRitz）融合深度学习与无网格方法，基于 Ritz 变分原理，通过再生核增强提升偏微分方程求解的精度与效率。方法在深度神经网络框架下引入再生核近似，针对一维问题实现显著的 $L^2$ 误差降低，展现出较高的推理效率与应用潜力。

实现过程如下：首先，深度 Ritz 方法以一维拉压杆模型为基础，传统 Ritz 方法通过预定义基函数近似位移场，而深度 Ritz 方法利用神经网络替代基函数，验证了其数值有效性。其次，伽辽金无网格法（RKPM）从弱形式出发，通过无网格离散化构造形状函数，结合拉格朗日乘子法强制边界条件并采用高斯积分法近似积分，显示出较好的导数拟合能力。最后，本文主要实现再生核近似增强深度 Ritz 法，通过再生核近似机制增强局部解表达能力，验证了方法的优越性。

算例验证以一维泊松方程为例，EFG-DeepRitz 方法在 10000 步训练后 $L^2$ 误差 0.002368 优于 RKPM（0.011709）和 DeepRitz（0.016862），$H^1$ 误差 0.058213 虽高于 RKPM（0.043137）但低于 DeepRitz（0.080057），误差分布显示其在中间区域精度更高，但导数误差波动较大，边界处需进一步优化。训练时间 244.76 秒长于 DeepRitz（34.79 秒）和 RKPM（无训练时间），但计算时间 0.01 秒远低于 RKPM（0.3031 秒），验证了方法在推理效率上的优势，为复杂偏微分方程求解提供了新颖且高效的数值工具。
增强深度 Ritz 方法通过引入 EFG 形状函数，增强了模型的表达能力，尤其在局部区域的逼近精度上表现优异。较低的 L2 和 H1 误差表明该方法更适合处理具有复杂梯度变化的偏微分方程问题。
